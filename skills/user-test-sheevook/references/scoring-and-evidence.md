...
**These rules govern the Product UX score.** The Marketing Output Quality score has its own rubric
in `output-quality.md` - do not apply this calibration table to an artifact.
...
Phase 0.7 loads `docs/reports/marketing-test-reports/learnings.md` (this skill's own store) plus the
standing suppression table in `harness.md`. If the current finding matches a prior
`verified-false-positive` entry (fuzzy title match), suppress it OR explicitly re-prove it. Suppress format in the report:
...
### Rule 6 — Domain-accuracy claims need a citation, not recall

A claim about a platform's limits, formats, algorithm, or ad objectives is **not** an observation —
it is a factual assertion about the outside world, and model recall of platform specifics is stale
by construction. It must cite `research/`, a `STABLE` ground-truth row, or a live `WebFetch`.
Otherwise it is `[UNVERIFIED]` and cannot be scored. See `domain-accuracy.md` — this is the rule
that keeps the platform experts from confidently "fixing" a correct value.

---

## Cognitive Load Simulation (personas forget)

AI remembers everything. Real users don't. **Experts are not exempt** — a fractional CMO juggling
three clients forgets a settings toggle she saw once, ten screens ago.
...
- Expertise changes *what* is retained, not *how much*: Rachel (media buyer) retains numbers and structure; Kofi (virality) retains the exact wording of a hook; Marisol (community) retains tone. All of them forget where the button was.
...
```
TASK: "Take one launch message from idea to scheduled, across 3 platforms"
SUCCESS CRITERIA: 3+ platform variants exist in `scheduled` state on the calendar
```

Not "explore the content features" — unmeasurable. Track:
